...
function initializeInputCaseEditSheet() {
  ...
  sheet.setColumnWidths(4, INPUT_FIELD_GROUPS.WORK_TIME.length, 150);

  sheet.getRange(INPUT_SHEET_LAYOUT.TITLE_CELL).setValue('料金改定案件 入力・編集');
...
function buildVerticalInputSection_(sheet, startRow, title, fields) {
  ...
  sheet.getRange(startRow + 1, 1, values.length + 1, 3).setBorder(true, true, true, true, true, true);
}
...
function buildWorkTimeInputSection_(sheet) {
  ...
  sheet.getRange(detailStartRow - 1, 1, rows.length + 1, fields.length).setBorder(true, true, true, true, true, true);
}
